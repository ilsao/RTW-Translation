...
所有光线追踪器都具备的一个共同点，就是拥有一个 `ray`（光线）类，以及一个计算沿着光线能看到什么颜色的算法。我们可以把光线看作一个函数：$P(t)=a+tb$ 。在这里，$P$ 是三维空间中沿着一条直线的三维位置（坐标）。$A$ 是光线起点（ray origin），$B$ 是光线方向（ray direction）。光线参数 t 是一个实数（在代码中为 `double` 类型）。代入不同的 t 值，P(t) 就会沿着光线移动该点。如果加上负的 t 值，你可以到达这条三维直线上的任意位置。而对于正的 t 值，你只能得到 $A$ 前方的部分，这也就是人们常说的射线（half-line，或直接叫 ray）。

[![表 2：线性插值](https://raytracing.github.io/images/fig-1.02-lerp.jpg)](https://raytracing.github.io/images/fig-1.02-lerp.jpg)

我们可以用一个类来表示光线的概念，并用一个我们取名为 `ray::at(t)` 的函数来表示 $P(t)$ 函数：
...
现在我们准备好迈入下一阶段，製作一个光线追踪器了。光线追踪器的核心原理，就是向像素投射光线，并计算在这些光线方向上所看到的颜色。涉及的步骤包括：
1. 计算从 " 视点（eye）" 穿过像素的光线
2. 确定光线与哪些物体相交
3. 计算最近交点处的颜色
...
接下来，我们将定义一个相机中心：一个三维空间中的点，所有场景光线都将从该点发出（此点也常被称为视点（eye point））。相机中心到视口中心的向量和视口正交。我们会先将视口到相机中心点的距离设为一个单位。这个距离通常被称为焦距（focal length）。
...
基于以上所有几何关係，以下是实作相机的代码。我们将存根一个函数 `ray_color(const ray& r)`，他负责返回一条给定场景光线的颜色——目前我们将它设定成永远回传黑色。
